...
"""Self-contained SAC Hover training helper for the notebook workflow.

This module is intended to provide a single-helper / single-notebook structure
for the final Hover SAC experiments. It contains the training, evaluation,
plotting, and statistics logic that was previously delegated to
``train_sac_report_plots.py``.
"""
...
class HoverTrainingConfig:
    """Configuration for the final SAC Hover runs."""
    ...
    def resolved_results_dir(self) -> Path:
        ...
        if self.results_dir is None:
            return self.resolved_project_root() / "results"
        return Path(self.results_dir).resolve()

    def resolved_save_name(self) -> str:
        """Return the save name used by the experiment."""
...
def make_env(cfg: HoverTrainingConfig, seed: int | None = None):
    ...
    env_id = env_map[cfg.env_name]
    env_kwargs = get_env_kwargs(cfg.env_name)
...
def seed_everything(seed: int) -> None:
    """Propagate the seed to the main random generators."""
...
def train_one_seed(cfg: HoverTrainingConfig, seed: int) -> str:
    """Train SAC for one seed and save model/evaluation logs."""
...
def evaluate_model(
    cfg: HoverTrainingConfig,
    model_path: str,
    eval_seeds: List[int],
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Evaluate a saved SAC model on deterministic rollouts."""
...
def bootstrap_ci_mean(values: np.ndarray, n_bootstrap: int = 5000, seed: int = 123) -> Tuple[float, float]:
...
def load_learning_curves(cfg: HoverTrainingConfig) -> Dict[int, Tuple[np.ndarray, np.ndarray]]:
    """Load SB3 EvalCallback evaluations.npz files for each seed."""

    curves = {}

    for seed in cfg.seeds:
        path = cfg.results_path("evaluations", f"{cfg.resolved_save_name()}_seed{seed}", "evaluations.npz")
        if not path.exists():
...
def plot_learning_curve(cfg: HoverTrainingConfig, out_path: Path) -> None:
    """Report Figure 1 style: per-seed curves + mean + bootstrap 95% CI."""
...
def plot_final_boxplot(cfg: HoverTrainingConfig, checkpoint_returns: Dict[int, np.ndarray], out_path: Path) -> None:
    """Report Figure 2 style: final 20-episode distribution per seed."""
    ...
    plt.title(f"SAC on {get_env_title(cfg.env_name)} — final eval (20 episodes per seed)")
...
def evaluate_saved_checkpoints(
    ...
    """Evaluate one saved checkpoint per seed and collect return stats."""
...
def build_checkpoint_path_map(cfg: HoverTrainingConfig, checkpoint_kind: str) -> Dict[int, str]:
    """Build the path of the checkpoint we want to evaluate for each seed."""
    ...
    if checkpoint_kind == "best_checkpoint":
        return {
            seed: str(cfg.results_path("best_models", f"{cfg.resolved_save_name()}_seed{seed}", "best_model.zip"))
            for seed in cfg.seeds
        }
...
def save_final_stats(
    ...
    """Save report-style final statistics as a CSV table."""
...
def make_all_plots_and_stats(cfg: HoverTrainingConfig) -> None:
    """Create report outputs for the final model and the best checkpoint."""
...
def train_all(
    ...
    """Run the final Hover training/evaluation workflow."""

    if cfg.env_name != "hover":
        raise ValueError(f"HoverTrainingConfig expects env_name='hover', got {cfg.env_name!r}")
...
def output_paths(cfg: HoverTrainingConfig) -> dict[str, Path]:
    """Return the main output files produced by the workflow."""
